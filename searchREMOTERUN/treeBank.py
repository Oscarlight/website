...
class TreeBank(object):
	...
	def isDetNum(self, pos_word):
		return contains(pos_word.split()[0], self.det_numList)
...
def deleteDetNum(lst):
	...
	tB = TreeBank()
	for wp in lst:
		if tB.isDetNum(wp) == None:
			result.append(wp)
	return result

def deleteToListWithPOS(node, deleteList):
	...
	lst = []
	pList = []
	for toDeletePair in deleteList:
		# print toDeletePair
		tempParent = findParentsOfWord(node, toDeletePair, [], node)
		if tempParent != []:
			if isinstance(tempParent, list):
				pList = pList + tempParent
			else:
				pList.append(findParentsOfWord(node, toDeletePair, [], node))
		# print pList
	# print pList
	if pList == []: # they are not been found.
...
def findNPAndNoun(lst):
	'''' input a list of Node object
		Return a list of top-level word pair list and a list of all NP nodes'''
	tB = TreeBank()
	nl = [] # top level word list
	for node in lst:
		if len(node.data.split()) == 2:
			if tB.isNoun(node.data) != None or tB.isPronoun(node.data) != None:
				nl.append(node)
